...
export default class ModalSay extends Vue {
  ...
  private get availableMessageTypes(): string[] {
    const list = this.situation.say.selectable_message_type_list.map(
      type => type.message_type.code
    )
    if (this.situation.creator.available_creator_say) {
      return list.concat(MESSAGE_TYPE.CREATOR_SAY)
    } else return list
  }
